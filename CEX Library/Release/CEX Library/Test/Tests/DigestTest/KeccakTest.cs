...
namespace VTDev.Projects.CEX.Test.Tests.DigestTest
{
    ...
    public class KeccakTest : ITest
    {
        ...
        private static readonly byte[][] _macKeys =
        {
            ...
            Hex.Decode("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" +
        ...
        private static readonly byte[] _truncKey = Hex.Decode("0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c");
        private static readonly byte[] _truncData = Hex.Decode("546573742057697468205472756e636174696f6e");
        private static readonly byte[] _trunc224 = Hex.Decode("f52bbcfd654264e7133085c5e69b72c3");
        private static readonly byte[] _trunc256 = Hex.Decode("745e7e687f8335280d54202ef13cecc6");
        private static readonly byte[] _trunc384 = Hex.Decode("fa9aea2bc1e181e47cbb8c3df243814d");
        private static readonly byte[] _trunc512 = Hex.Decode("04c929fead434bba190dacfa554ce3f5");
        private static readonly byte[] _xtremeData = Hex.Decode("61626364656667686263646566676869636465666768696a6465666768696a6b65666768696a6b6c666768696a6b6c6d6768696a6b6c6d6e68696a6b6c6d6e6f");
        #endregion
        ...
        private void DigestTest(IDigest Digest, string[] Expected)
        {
            ...
            for (int i = 0; i != _messages.Length; i++)
            {
                ...
                {
                    byte[] data = Hex.Decode(_messages[i]);
                    Digest.BlockUpdate(data, 0, data.Length);
                }

                Digest.DoFinal(hash, 0);

                if (Compare.AreEqual(Hex.Decode(Expected[i]), hash) == false)
                    throw new Exception("Keccak: Expected hash is not equal! Expected: " + Expected[i] + "Received: " + Hex.ToString(hash));
            }
            ...
            if (Compare.AreEqual(Hex.Decode(Expected[_messages.Length]), hash) == false)
                throw new Exception("Keccak: Expected hash is not equal! Expected: " + Expected[_messages.Length] + "Received: " + Hex.ToString(hash));
            ...
            if (Compare.AreEqual(Hex.Decode(Expected[_messages.Length + 1]), hash) == false)
                throw new Exception("Keccak: Expected hash is not equal! Expected: " + Expected[_messages.Length + 1] + "Received: " + Hex.ToString(hash));
            ...
            if (Compare.AreEqual(Hex.Decode(expected[messages.Length + 2]), hash) == false)
        ...
        private void DoFinalTest(IDigest digest)
        {
            ...
            for (int i = 0; i <= digest.DigestSize; ++i)
            {
                ...
                if (Compare.AreEqual(expected, outBytes) == false)
                    throw new Exception("Keccak DoFinal: Expected hash is not equal! Expected: " + Hex.ToString(expected) + "Received: " + Hex.ToString(outBytes));
            }
        }
        ...
        private void HMACTest(IDigest Digest, String[] Expected, byte[] TruncExpected)
        {
            ...
            for (int i = 0; i != _macKeys.Length; i++)
            {
                ...
                byte[] mData = Hex.Decode(_macData[i]);
                byte[] macV = new byte[mac.DigestSize];

                mac.BlockUpdate(mData, 0, mData.Length);
                mac.DoFinal(macV, 0);

                if (Compare.AreEqual(Hex.Decode(Expected[i]), macV) == false)
                    throw new Exception("Keccak HMAC: Expected hash is not equal! Expected: " + Expected[i] + "Received: " + Hex.ToString(macV));
            }
