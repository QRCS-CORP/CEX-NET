...
namespace VTDev.Projects.CEX.Test.Tests.CipherTest
{
    ...
    public class AesFipsTest : ITest
    {
        ...
        private void VectorTest(byte[] Key, byte[] Input, byte[] Output)
        {
            ...
            using (RDX engine = new RDX())
            {
                ...
                if (Compare.AreEqual(outBytes, Output) == false)
                    throw new Exception("AES: Encrypted arrays are not equal! Expected: " + Hex.ToString(Output) + "Received: " + Hex.ToString(outBytes));

                engine.Initialize(false, new KeyParams(Key));
                engine.Transform(Output, outBytes);

                if (Compare.AreEqual(outBytes, Input) == false)
                    throw new Exception("AES: Decrypted arrays are not equal! Expected: " + Hex.ToString(Input) + "Received: " + Hex.ToString(outBytes));
            }
        }
        ...
        private void MonteCarloTest(byte[] Key, byte[] Input, byte[] Output)
        {
            ...
            if (Compare.AreEqual(outBytes, Output) == false)
                throw new Exception("AES MonteCarlo: Arrays are not equal! Expected: " + Hex.ToString(Output) + "Received: " + Hex.ToString(outBytes));
            ...
            if (Compare.AreEqual(outBytes, Input) == false)
                throw new Exception("AES MonteCarlo: Arrays are not equal! Expected: " + Hex.ToString(Input) + "Received: " + Hex.ToString(outBytes));
        }
        #endregion
    }
}
