    ...
    public class SerpentTest : ITest
    {
        ...
        public string Test()
        {
            try
            {
                ...
                for (int i = 0; i < keyStr.Length; i += 32)
                {
                    ...
                    pln = Hex.Decode(plnStr.Substring(i, 32));
                    ...
                    if (doMonte)
                    {
                        mnt = Hex.Decode(mntStr.Substring(i, 32));
                        Array.Reverse(mnt);
                        // monte carlo 100 rounds
                        MonteCarloTest(key, pln, mnt);
                        rcount += 100;
                        // 1000 rounds
                        mnt = Hex.Decode(mnt1kStr.Substring(i, 32));
                        Array.Reverse(mnt);
        ...
        private void KeyTest()
        {
            ...
            if (Compare.AreEqual(inBytes, decBytes) == false)
                throw new Exception("Serpent: Decrypted arrays are not equal! Expected: " + Hex.ToString(inBytes) + "Received: " + Hex.ToString(decBytes));
        }
        ...
        private void MonteCarloTest(byte[] Key, byte[] Input, byte[] Output, int Count = 100)
        {
            ...
            if (Compare.AreEqual(outBytes, Output) == false)
                throw new Exception("Serpent MonteCarlo: Arrays are not equal! Expected: " + Hex.ToString(Output) + "Received: " + Hex.ToString(outBytes));
        }
        ...
        private void VectorTest(byte[] Key, byte[] Input, byte[] Output)
        {
            ...
            if (Compare.AreEqual(Output, outBytes) == false)
                throw new Exception("SerpentVector: Encrypted arrays are not equal! Expected: " + Hex.ToString(Output) + "Received: " + Hex.ToString(outBytes));
            ...
            if (Compare.AreEqual(Input, outBytes) == false)
                throw new Exception("SerpentVector: Decrypted arrays are not equal! Expected: " + Hex.ToString(Input) + "Received: " + Hex.ToString(outBytes));
        }
        #endregion
    }
}
