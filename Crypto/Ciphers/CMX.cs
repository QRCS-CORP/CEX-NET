...
{
    public sealed class CMX : IBlockCipher, IDisposable
    {
        #region Constants
		...
		private uint[] _exKey = new uint[24 * 4];
        ...
        public void DecryptBlock(byte[] Input, byte[] Output)
        {
            if (this.BlockSize == 16)
                Process16(Input, 0, Output, 0);
            else
                Process32(Input,0,  Output, 0);
        }
        ...
        {
            if (this.BlockSize == 16)
                Process16(Input, InOffset, Output, OutOffset);
            else
                Process32(Input, InOffset, Output, OutOffset);
        }
        ...
        public void Init(bool Encryption, KeyParams KeyParam)
		{
			...
			SetKey(Encryption, KeyParam.Key);
        ...
        #region Transform
        private int Process16(byte[] Input, int InOffset, byte[] Output, int OutOffset)
        {
            for (int i = 0; i < 4; i++)
            {
                state[i] = BytesToWord(Input, InOffset + (i * 4));
                state[i] ^= kw[i];
            }
            ...
            WordToBytes(state[2], Output, OutOffset);
            WordToBytes(state[3], Output, OutOffset + 4);
            WordToBytes(state[0], Output, OutOffset + 8);
            WordToBytes(state[1], Output, OutOffset + 12);

            return BLOCK_SIZE;
        }

        private int Process32(byte[] Input, int InOffset, byte[] Output, int OutOffset)
        {
            for (int i = 0; i < 4; i++)
            {
                state[i] = BytesToWord(Input, InOffset + (i * 4));
        ...
        #region Helpers
        private uint RightRotate(uint X, int S)
        {
            return ((X >> S) + (X << (32 - S)));
        }
        ...
        private void RolDq(int Rot, uint[] Ki, int InOffset, uint[] Ko, int OutOffset)
        {
            Ko[0 + OutOffset] = (Ki[0 + InOffset] << Rot) | (Ki[1 + InOffset] >> (32 - Rot));
            Ko[1 + OutOffset] = (Ki[1 + InOffset] << Rot) | (Ki[2 + InOffset] >> (32 - Rot));
            Ko[2 + OutOffset] = (Ki[2 + InOffset] << Rot) | (Ki[3 + InOffset] >> (32 - Rot));
            Ko[3 + OutOffset] = (Ki[3 + InOffset] << Rot) | (Ki[0 + InOffset] >> (32 - Rot));
            Ki[0 + InOffset] = Ko[0 + OutOffset];
            Ki[1 + InOffset] = Ko[1 + OutOffset];
            Ki[2 + InOffset] = Ko[2 + OutOffset];
            Ki[3 + InOffset] = Ko[3 + OutOffset];
        }

        private void DecRolDq(int Rot, uint[] Ki, int InOffset, uint[] Ko, int OutOffset)
        {
            Ko[2 + OutOffset] = (Ki[0 + InOffset] << Rot) | (Ki[1 + InOffset] >> (32 - Rot));
            Ko[3 + OutOffset] = (Ki[1 + InOffset] << Rot) | (Ki[2 + InOffset] >> (32 - Rot));
            Ko[0 + OutOffset] = (Ki[2 + InOffset] << Rot) | (Ki[3 + InOffset] >> (32 - Rot));
            Ko[1 + OutOffset] = (Ki[3 + InOffset] << Rot) | (Ki[0 + InOffset] >> (32 - Rot));
            Ki[0 + InOffset] = Ko[2 + OutOffset];
            Ki[1 + InOffset] = Ko[3 + OutOffset];
            Ki[2 + InOffset] = Ko[0 + OutOffset];
            Ki[3 + InOffset] = Ko[1 + OutOffset];
        }

        private void RolDqo32(int rot, uint[] ki, int ioff, uint[] ko, int ooff)
        {
        ...
        private void DecRolDqo32(int rot, uint[] ki, int ioff, uint[] ko, int ooff)
        {
        ...
        {
            return (UInt32)((Src[Offset] << 24) |
                            (Src[Offset + 1] << 16) |
                            (Src[Offset + 2] << 8) |
                            (Src[Offset + 3]));
        }

        private void WordToBytes(UInt32 Word, byte[] Dst, Int32 DstOff)
        {
        ...
        private void CF2(uint[] State, uint[] Key, int Offset)
        {
            uint t1, t2, u, v;// process 2 rounds

            t1 = State[0] ^ Key[0 + Offset];
            u = SB4404[(byte)t1];
            u ^= SB3033[(byte)(t1 >> 8)];
            u ^= SB0222[(byte)(t1 >> 16)];
            u ^= SB1110[(byte)(t1 >> 24)];

            t2 = State[1] ^ Key[1 + Offset];
            v = SB1110[(byte)t2];
            v ^= SB4404[(byte)(t2 >> 8)];
            v ^= SB3033[(byte)(t2 >> 16)];
            v ^= SB0222[(byte)(t2 >> 24)];
            ...
            t1 = State[2] ^ Key[2 + Offset];
            u = SB4404[(byte)t1];
            u ^= SB3033[(byte)(t1 >> 8)];
            u ^= SB0222[(byte)(t1 >> 16)];
            u ^= SB1110[(byte)(t1 >> 24)];

            t2 = State[3] ^ Key[3 + Offset];
            v = SB1110[(byte)t2];
            v ^= SB4404[(byte)(t2 >> 8)];
            v ^= SB3033[(byte)(t2 >> 16)];
            v ^= SB0222[(byte)(t2 >> 24)];
        ...
        private void CFLs(uint[] State, uint[] Key, int Offset)
        {
            // FL-function
            State[1] ^= LeftRotate(State[0] & Key[0 + Offset], 1);
            State[0] ^= Key[1 + Offset] | State[1];

            State[2] ^= Key[3 + Offset] | State[3];
            State[3] ^= LeftRotate(Key[2 + Offset] & State[2], 1);
        }

        private void SetKey(bool Encryption, byte[] Key)
        {
            uint[] k = new uint[8];
            uint[] ka = new uint[4];
            uint[] kb = new uint[4];
            ...
            switch (Key.Length)
            {
                case 16:
                    _keyIs128 = true;
                    k[0] = BytesToWord(Key, 0);
                    k[1] = BytesToWord(Key, 4);
                    k[2] = BytesToWord(Key, 8);
                    k[3] = BytesToWord(Key, 12);
                    k[4] = k[5] = k[6] = k[7] = 0;
                    break;
                case 24:
                    k[0] = BytesToWord(Key, 0);
                    ...
                    k[2] = BytesToWord(Key, 8);
                    k[3] = BytesToWord(Key, 12);
                    k[4] = BytesToWord(Key, 16);
                    k[5] = BytesToWord(Key, 20);
                    k[6] = ~k[4];
                    k[7] = ~k[5];
                    _keyIs128 = false;
                    break;
                case 32:
                    ...
                    k[4] = BytesToWord(Key, 16);
                    k[5] = BytesToWord(Key, 20);
                    k[6] = BytesToWord(Key, 24);
                    k[7] = BytesToWord(Key, 28);
                    _keyIs128 = false;
                    break;
                default:
            ...
            if (_keyIs128)
            {
                if (Encryption)
                {
                    ...
                    kw[2] = k[2];
                    kw[3] = k[3];
                    RolDq(15, k, 0, _exKey, 4);
                    RolDq(30, k, 0, _exKey, 12);
                    RolDq(15, k, 0, t, 0);
                    _exKey[18] = t[2];
                    _exKey[19] = t[3];
                    RolDq(17, k, 0, ke, 4);
                    RolDq(17, k, 0, _exKey, 24);
                    RolDq(17, k, 0, _exKey, 32);
                    /* KA dependant keys */
                    _exKey[0] = ka[0];
                    _exKey[1] = ka[1];
                    _exKey[2] = ka[2];
                    _exKey[3] = ka[3];
                    RolDq(15, ka, 0, _exKey, 8);
                    RolDq(15, ka, 0, ke, 0);
                    RolDq(15, ka, 0, t, 0);
                    _exKey[16] = t[0];
                    _exKey[17] = t[1];
                    RolDq(15, ka, 0, _exKey, 20);
                    RolDqo32(34, ka, 0, _exKey, 28);
                    RolDq(17, ka, 0, kw, 4);
                ...
                { // decryption
                    ...
                    kw[6] = k[2];
                    kw[7] = k[3];
                    DecRolDq(15, k, 0, _exKey, 28);
                    DecRolDq(30, k, 0, _exKey, 20);
                    DecRolDq(15, k, 0, t, 0);
                    _exKey[16] = t[0];
                    _exKey[17] = t[1];
                    DecRolDq(17, k, 0, ke, 0);
                    DecRolDq(17, k, 0, _exKey, 8);
                    DecRolDq(17, k, 0, _exKey, 0);
                    /* KA dependant keys */
                    _exKey[34] = ka[0];
                    _exKey[35] = ka[1];
                    _exKey[32] = ka[2];
                    _exKey[33] = ka[3];
                    DecRolDq(15, ka, 0, _exKey, 24);
                    DecRolDq(15, ka, 0, ke, 4);
                    DecRolDq(15, ka, 0, t, 0);
                    _exKey[18] = t[2];
                    _exKey[19] = t[3];
                    DecRolDq(15, ka, 0, _exKey, 12);
                    DecRolDqo32(34, ka, 0, _exKey, 4);
                    RolDq(17, ka, 0, kw, 0);
                }
            }
            else
            { // 192bit or 256bit
                ...
                if (Encryption)
                {
                    ...
                    kw[2] = k[2];
                    kw[3] = k[3];
                    RolDqo32(45, k, 0, _exKey, 16);
                    RolDq(15, k, 0, ke, 4);
                    RolDq(17, k, 0, _exKey, 32);
                    RolDqo32(34, k, 0, _exKey, 44);
                    /* KR dependant keys */
                    RolDq(15, k, 4, _exKey, 4);
                    RolDq(15, k, 4, ke, 0);
                    RolDq(30, k, 4, _exKey, 24);
                    RolDqo32(34, k, 4, _exKey, 36);
                    /* KA dependant keys */
                    RolDq(15, ka, 0, _exKey, 8);
                    RolDq(30, ka, 0, _exKey, 20);
                    /* 32bit rotation */
                    ke[8] = ka[1];
                    ke[9] = ka[2];
                    ke[10] = ka[3];
                    ke[11] = ka[0];
                    RolDqo32(49, ka, 0, _exKey, 40);
                    ...
                    _exKey[2] = kb[2];
                    _exKey[3] = kb[3];
                    RolDq(30, kb, 0, _exKey, 12);
                    RolDq(30, kb, 0, _exKey, 28);
                ...
                { // decryption
                    ...
                    kw[6] = k[2];
                    kw[7] = k[3];
                    DecRolDqo32(45, k, 0, _exKey, 28);
                    DecRolDq(15, k, 0, ke, 4);
                    DecRolDq(17, k, 0, _exKey, 12);
                    DecRolDqo32(34, k, 0, _exKey, 0);
                    /* KR dependant keys */
                    DecRolDq(15, k, 4, _exKey, 40);
                    DecRolDq(15, k, 4, ke, 8);
                    DecRolDq(30, k, 4, _exKey, 20);
                    DecRolDqo32(34, k, 4, _exKey, 8);
                    /* KA dependant keys */
                    DecRolDq(15, ka, 0, _exKey, 36);
                    DecRolDq(30, ka, 0, _exKey, 24);
                    /* 32bit rotation */
                    ke[2] = ka[1];
                    ke[3] = ka[2];
                    ke[0] = ka[3];
                    ke[1] = ka[0];
                    DecRolDqo32(49, ka, 0, _exKey, 4);
                    ...
                    _exKey[44] = kb[2];
                    _exKey[45] = kb[3];
                    DecRolDq(30, kb, 0, _exKey, 32);
                    DecRolDq(30, kb, 0, _exKey, 16);
        ...
        private static readonly uint[] SB1110 = new uint[]{
        ...
        private static readonly uint[] SB4404 = new uint[]{
        ...
        private static readonly uint[] SB0222 = new uint[]{
        ...
        private static readonly uint[] SB3033 = new uint[]{
